...
export async function runSendBatch(): Promise<SendBatchResult> {
  ...
  if (remaining === 0) {
    return {
      ...
      capped: true, remaining_today: 0, errors: [],
      message: `Daily limit of ${dailyLimit} already reached (${sentToday} sent today)`,
    }
  }
