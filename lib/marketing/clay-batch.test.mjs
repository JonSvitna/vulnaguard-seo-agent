...
test('accepts an origami source and derives its own category/source_detail', () => {
  const lead = normalizeClayLead({ ...validLead, source: 'origami' });
  assert.equal(lead.source, 'origami');
  assert.equal(lead.source_detail, 'scheduled_origami_run');
  assert.equal(lead.category, 'origami_leads');
});

test('rejects an unsupported source', () => {
  assert.throws(() => normalizeClayLead({ ...validLead, source: 'zoominfo' }), TypeError);
});
...
test('rejects email addresses beyond mailbox length limits', () => {
  assert.throws(
    () => normalizeClayLead({ ...validLead, email: `${'a'.repeat(65)}@example.com` }),
    /email/i,
  );

  const longDomain = `${'a'.repeat(63)}.${'b'.repeat(63)}.${'c'.repeat(62)}`;
  const overlongEmail = `${'d'.repeat(64)}@${longDomain}`;
  assert.equal(overlongEmail.length, 255);
  assert.throws(() => normalizeClayLead({ ...validLead, email: overlongEmail }), /email/i);
});
