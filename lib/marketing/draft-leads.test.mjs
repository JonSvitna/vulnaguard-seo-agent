...
import { draftLeadIds, summarizeDraftLeadResults } from './draft-leads.ts';
...
test('returns a retryable skip when an initially email-less lead gains an email before locking', async () => {
  let changed = false;
  let drafts = 0;
  const { deps, state } = fakeDependencies([lead(25, { contact_email: null })], {
    beforeTransaction: (database) => {
      if (!changed) database.leads[0].contact_email = 'new@example.com';
      changed = true;
    },
    draftSequence: async () => {
      drafts++;
  ...
  assert.deepEqual(await draftLeadIds([25], deps), [
    { leadId: 25, status: 'skipped', reason: 'lead_changed_retry' },
  ]);
  assert.equal(drafts, 0);
  assert.equal(state.leads[0].status, 'discovered');
  assert.equal(state.sequences.length, 0);
...
test('summarizes drafted, failed, and skipped reason counts for route telemetry', () => {
  assert.deepEqual(summarizeDraftLeadResults([
    { leadId: 1, status: 'drafted' },
    { leadId: 2, status: 'skipped', reason: 'already_drafted' },
    { leadId: 3, status: 'skipped', reason: 'already_drafted' },
    { leadId: 4, status: 'skipped', reason: 'lead_changed_retry' },
    { leadId: 5, status: 'failed', reason: 'persistence_failed' },
  ]), {
    drafted: 1,
    errors: 1,
    skipped: 3,
    skipped_reasons: { already_drafted: 2, lead_changed_retry: 1 },
  });
});
